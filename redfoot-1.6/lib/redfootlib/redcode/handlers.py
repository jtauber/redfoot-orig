...
from redfootlib.server import register_module, get_module
...
from redfootlib.xml.dom import Node, NodeList, TextNode
from redfootlib.xml.dom import HTMLElementNode, ElementNode
from redfootlib.xml.dom import EvalNode, EncodedEvalNode
...
class ElementHandler(HandlerBase):
    def __init__(self, parser, parent, name, atts):
        ...
        for key in atts.keys():
            e_atts[local(key)] = parse_attribute(atts[key])

        name = local(name)
        if name=="html":
            self.element = HTMLElementNode(name, e_atts)
            # TODO: set doctype
        else:
    ...
    def child(self, name, atts):
        ...
        if name==MODULE:
            if atts.has_key('bases'):
                ...
                base_classes = (__import__('redfootlib.server.module', self.globals, self.locals, ['ParentModule']).ParentModule, )
            eh = ModuleHandler(self.parser, self, atts['name'], base_classes)
        elif name==APP:
            base_classes = (__import__('redfootlib.server.module', self.globals, self.locals, ['App']).App, )
            eh = ModuleHandler(self.parser, self, atts['name'], base_classes)
...
class ModuleHandler(HandlerBase):
    ...
    def child(self, name, atts):
        if name==FACET:
            nh = Facet(self.parser, self, name, atts)
...
class Visit(ElementHandler):
    ...
    def child(self, name, atts):
        if self.first_child:
            ...
            if name==CALLBACK:
                nh = Facet(self.parser, self, CALLBACK, atts)
