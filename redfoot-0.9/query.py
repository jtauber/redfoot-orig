...
class QueryStore:
    ...
    def comment(self, subject):
        c = self.get(subject, COMMENT, None)
        if len(c) > 0:
    ...
    def isOfType(self, resource, type):
        for s in model.store.get(resource, TYPE, None):
            if s[2] == type:
                return 1
        return 0
    ...
    def transitiveSubTypes(self, type):
        ...
        for subclassStatement in self.store.get(None, SUBCLASSOF, type):
